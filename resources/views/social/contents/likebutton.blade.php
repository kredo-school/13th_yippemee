        ...
        @if($social_post->isLiked())
            <form action="{{ route('social.posts.unlike', $social_post->id) }}" method="POST">
                ...
                @if($social_post->likes->count() > 0)
                    <button type="button" class="btn shadow-none ps-0" data-bs-toggle="modal" data-bs-target="#likes-modal">{{ $social_post->likes->count() }}</button>
                @endif
            </form>
        @else
            <form action="{{ route('social.posts.like', $social_post->id) }}" method="POST">
                @csrf
                <button type="submit" class="heart-button"><i class="fa-regular fa-heart"></i></button>
                @if($social_post->likes->count() > 0)
                    <button type="button" class="btn shadow-none ps-0 like-button" data-bs-toggle="modal" data-bs-target="#likes-modal">{{ $social_post->likes->count() }}</button>
                @endif
            </form>
        @endif
